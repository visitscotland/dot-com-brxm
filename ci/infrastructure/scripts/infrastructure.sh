...
defaultSettings() {
  ...
  if [ -z "$VS_CONTAINER_NAME" ]&&[ "$VS_BRANCH_NAME" != "branch-not-found" ]; then
    ...
    VS_CONTAINER_NAME_RFC1034="$(echo "$VS_CONTAINER_NAME_SHORTEST" | tr '[:upper:]' '[:lower:]' | cut -c 1-54 | sed -e 's/[^a-z0-9]/-/g' -e 's/^-*//' -e 's/-*$//')-$(echo -n "$VS_CONTAINER_NAME_SHORTEST" | md5sum | cut -c 1-8)"
    VS_CONTAINER_NAME="$VS_CONTAINER_NAME_BASE""_""$VS_CONTAINER_NAME_SHORT"
  fi
