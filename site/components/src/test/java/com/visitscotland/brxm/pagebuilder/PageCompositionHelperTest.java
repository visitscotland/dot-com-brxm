...
class PageCompositionHelperTest {
    ...
    private PageTemplateAssembler pageTemplateAssembler;

    @BeforeEach
    void setUp() {
        resourceBundleService = mock(ResourceBundleService.class);
        pageTemplateAssembler = mock(PageTemplateAssembler.class);
        request = mock(HstRequest.class);

        helper = new PageCompositionHelper(resourceBundleService, pageTemplateAssembler, request);
    }
    ...
    void When_addGlobalLabel_then_storeLabelInGlobalBundle() {
        ...
        when(resourceBundleService.getResourceBundle(any(), eq("testKey"), isNull(Locale.class)))
                .thenReturn("labelValue");
        when(resourceBundleService.getResourceBundle(any(), eq("testKey2"), isNull(Locale.class)))
