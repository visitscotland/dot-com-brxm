...
public class ListicleFactory {
    ...
    private FlatLink processMainProduct(Locale locale, HippoCompound link, ListicleModule module){
        if (link == null) {
            contentLogger.warn("The ListicleItem {} doesn't contain a main product", module.getDocumentPath());
            ...
            if (cmsLink.getLink() instanceof Linkable){
                ...
                contentLogger.warn("The ListicleItem {} is pointing to a document that is not a page ", module.getDocumentPath());
    ...
    public List<ListicleModule> generateItems(Locale locale, Listicle listicle) {
        ...
        if (listicleItems != null && !listicleItems.isEmpty()){
            ...
            contentLogger.warn("The listicle page {} does not have any modules published", listicle.getPath());
        }
        return items;
    }
}
